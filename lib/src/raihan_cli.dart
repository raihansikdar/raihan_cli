...
void runCli(List<String> args) async {
  ...
  if (architecture == 'clean') {
    ...
    // state-specific folder inside presentation
    if (stateManagement == 'bloc') {
      folders.add('$basePath/presentation/bloc');
    } else if (stateManagement == 'provider') {
      folders.add('$basePath/presentation/provider');
    ...
    createdAnything = createFile(
      ...
      '''
abstract class ${pascalFeature}RemoteDataSource {
  Future<dynamic> fetch${pascalFeature}s();
}
''',
    ) ||
        createdAnything;
...
class ${pascalFeature}RemoteDataSourceImpl implements ${pascalFeature}RemoteDataSource {
  // final Dio _dio;   // inject your http client
  const ${pascalFeature}RemoteDataSourceImpl();

  @override
  Future<dynamic> fetch${pascalFeature}s() async {
...
''',
    ...
    createdAnything = createFile(
      ...
      '''
abstract class ${pascalFeature}LocalDataSource {
  Future<dynamic> getCached${pascalFeature}s();
  Future<void> cache${pascalFeature}s(dynamic data);
}
''',
    ) ||
        createdAnything;
...
class ${pascalFeature}Model {
  ...
  const ${pascalFeature}Model({required this.id, required this.name});

  factory ${pascalFeature}Model.fromJson(Map<String, dynamic> json) {
    return ${pascalFeature}Model(
      id: json['id'] as int,
...
class ${pascalFeature}Entity {
  ...
  const ${pascalFeature}Entity({required this.id, required this.name});
}
''',
    ) ||
        createdAnything;
...
class ${pascalFeature}UseCase {
  ...
  Future<Either<Exception, List<dynamic>>> execute() => repository();
}
''',
    ) ||
        createdAnything;
...
import '${snakeFeature}_event.dart';
import '${snakeFeature}_state.dart';
import '../../domain/use_case/${snakeFeature}_use_case.dart';

class ${pascalFeature}Bloc extends Bloc<${pascalFeature}Event, ${pascalFeature}State> {
  final ${pascalFeature}UseCase useCase;

  ${pascalFeature}Bloc(this.useCase) : super(${pascalFeature}Initial()) {
    on<Load${pascalFeature}Event>(_onLoad);
  }

  Future<void> _onLoad(
    Load${pascalFeature}Event event,
    Emitter<${pascalFeature}State> emit,
  ) async {
    emit(${pascalFeature}Loading());
    final result = await useCase.execute();
    result.fold(
      (failure) => emit(${pascalFeature}Error(failure.toString())),
      (data) => emit(${pascalFeature}Loaded(data)),
    );
  }
}
''') || createdAnything;
...
class Load${pascalFeature}Event extends ${pascalFeature}Event {}
''') || createdAnything;
...
class ${pascalFeature}Initial extends ${pascalFeature}State {}
class ${pascalFeature}Loading extends ${pascalFeature}State {}
class ${pascalFeature}Loaded extends ${pascalFeature}State {
  final List<dynamic> data;
  ${pascalFeature}Loaded(this.data);
}
class ${pascalFeature}Error extends ${pascalFeature}State {
  final String message;
  ${pascalFeature}Error(this.message);
}
''') || createdAnything;
...
final ${snakeFeature}UseCaseProvider = Provider<${pascalFeature}UseCase>((ref) => throw UnimplementedError());

final ${snakeFeature}Provider = FutureProvider<List<dynamic>>((ref) async {
  final useCase = ref.read(${snakeFeature}UseCaseProvider);
  final result = await useCase.execute();
...
class ${pascalFeature}Notifier extends StateNotifier<AsyncValue<List<dynamic>>> {
  final ${pascalFeature}UseCase _useCase;

  ${pascalFeature}Notifier(this._useCase) : super(const AsyncValue.loading()) {
    load();
  }
...
final ${snakeFeature}NotifierProvider = StateNotifierProvider<${pascalFeature}Notifier, AsyncValue<List<dynamic>>>((ref) {
  final useCase = ref.read(${snakeFeature}UseCaseProvider);
  return ${pascalFeature}Notifier(useCase);
});
''',
      ) ||
          createdAnything;
...
class ${pascalFeature}Controller extends GetxController {
  final ${pascalFeature}UseCase useCase;
  ${pascalFeature}Controller(this.useCase);
...
''',
    ...
    createdAnything = createFile(
      '$basePath/${snakeFeature}_injection_container.dart',
      '''
// Example with get_it (add "get_it" to pubspec.yaml if you use it)
// import 'package:get_it/get_it.dart';
// final sl = GetIt.instance;

// void setup${pascalFeature}Dependencies() {
//   // Register data sources, repository, use-case, state-manager...
// }
''',
    ) ||
        createdAnything;
    ...
    // -------------------------------------------------
    // MVC / MVVM (unchanged – only folder/file creation)
    // -------------------------------------------------
    List<String> folders = [
      '$basePath/model',
      '$basePath/views/screen',
...
void _saveConfig(Map<String, String> config) {
  ...
  try {
    ...
    final buffer = StringBuffer();
    config.forEach((k, v) => buffer.writeln('$k=$v'));
    configFile.writeAsStringSync(buffer.toString());
  } catch (e) {
    throw Exception('Failed to save config to $configFilePath: $e');
  }
